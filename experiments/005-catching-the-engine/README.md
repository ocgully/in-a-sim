...
|---|---|---|---|---|
| T1 | **Same-tick decisions** (author's idea): many quantum outcomes committed in the same tick, as a batching optimisation | timestamps of *independent* quantum events (decays, photon detections) pile up at tick boundaries; excess exact coincidences between unrelated detectors | direct test reaches ticks ≳ ~2.5 ps (best detector jitter 2.7 ps FWHM, Korzh et al. 2020); a model-dependent indirect bound puts any fundamental period T < 10⁻³³ s (Wendel, Martínez & Bojowald 2020 PRL); no prior proposal of the timestamp test found | Ep 2 (the tick) · [sim](sims/tick_batching.py) |
| T2 | **Finite rollback / pending-state window** | correlations vanish beyond some delay | ≥ 10 s (lab memory); ≥ ~11 Gyr for which-path state of isolated photons (GRAVITY+ 2024) | Ep 1, Exp 003 |
...
| T4 | **A global server clock** (preferred frame) | direction- or velocity-dependent physics; FTL would expose it | light speed doesn't depend on direction to ~10⁻¹⁸; velocity-dependent (preferred-frame) terms bounded to ~10⁻¹⁴ (Nagel et al. 2015) | Roadmap A (FTL) |
...
| T6 | **Lag spikes** (a region stalls, then catches up) | glitches that **sweep across** a network of atomic clocks (a *global* stall would be invisible from inside) | GPS/atomic-clock networks already search for exactly such sweeping glitches (for dark-matter "domain walls"): Roberts et al. 2017; Wcisło et al. 2016, 2018. None seen | Ep 2 |
...
| T8 | **Precision limits far from an origin** (float or integer overflow) | "constants" drifting with distance or direction | fine-structure-constant dipole: 4.2σ claim (Webb 2011) is contested by calibration systematics; ESPRESSO (Murphy 2022) finds 1.3 ± 1.3 ppm, consistent with no change: tentative at best | literature survey |
| T9 | **A pseudo-random number generator** | statistical structure in quantum randomness (periodicity, correlations) | no finite statistical test proves randomness (Herrero-Collantes & Garcia-Escartin 2017); Bell-certified randomness (Pironio 2010) assumes measurement settings are independent of the source, so an engine that also chooses the settings could still hide a PRNG | Ep 1 |
